...
public class ProductoService {
    ...
    public PaginaDTO<ProductoCardDTO> listarCatalogo(
        ...
        Page<Producto> productos =
                productoRepository.buscarCatalogo(
                        categoriaId,
                        marcaId,
                        fit,
                        query,
        ...
        Set<Long> favoritosIds = obtenerFavoritosDelUsuario();
    ...
    public ProductoDetalleDTO obtenerPorSlug(String slug) {
        ...
        Producto producto =
                productoRepository.findBySlugParaCatalogo(slug)
                        .orElseThrow(() ->
                                new ResponseStatusException(
                                        HttpStatus.NOT_FOUND,
                                        "Producto no encontrado: " + slug
                                )
                        );

        Set<Long> favoritosIds =
                obtenerFavoritosDelUsuario();

        ProductoDetalleDTO dto =
                ProductoDetalleDTO.fromEntity(producto);

        dto.setFavorito(
                favoritosIds.contains(producto.getId())
        );
    ...
    public PaginaDTO<ProductoAdminDTO> listarParaAdmin(
            String q,
            int pagina
    ) {
        ...
        Pageable pageable =
                PageRequest.of(
                        pagina,
                        TAMANIO_PAGINA_ADMIN
                );

        Page<Producto> productos =
                productoRepository.buscarParaAdmin(
                        query,
                        pageable
                );
    ...
    public ProductoAdminDTO obtenerPorIdParaAdmin(Long id) {
        ...
        Producto producto =
                productoRepository.findByIdParaAdmin(id)
                        .orElseThrow(() ->
                                new ResponseStatusException(
                                        HttpStatus.NOT_FOUND,
    ...
    public ProductoAdminDTO crear(
        ...
        String slug =
                generarSlugUnico(request.nombre());
    ...
    public ProductoEstadisticasDTO obtenerEstadisticas() {
        ...
        return ProductoEstadisticasDTO.fromProjection(
                productoRepository.obtenerEstadisticasAdmin()
        );
    }
    ...
    private Set<Long> obtenerFavoritosDelUsuario() {
        ...
        return favoritoRepository
                .findProductoIdsByUsuarioId(usuarioId);
    }


    private ProductoCardDTO convertirCard(
            Producto producto,
            Set<Long> favoritosIds
    ) {

        ProductoCardDTO dto =
                ProductoCardDTO.fromEntity(producto);
    ...
    private ProductoAdminDTO toAdminDTO(
            Producto producto
    ) {
    ...
    private String generarSlugUnico(String nombre) {
        ...
        while (
                productoRepository.existsBySlug(slug)
        ) {
